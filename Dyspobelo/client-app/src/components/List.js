...
const getCoordinatesFromAddress = async (address, postalCode) => {
	const formattedAddress = encodeURIComponent(
		`${address}, ${postalCode}, Warsaw, Poland`
	);
	const viewbox = "20.7122,52.0497,21.3122,52.4097";
	const response = await fetch(
		`https://nominatim.openstreetmap.org/search?q=${formattedAddress}&format=json&addressdetails=1&limit=1&viewbox=${viewbox}&bounded=1`
	);
				...
				{loading ? (
					...
					applySorting(
						...
						<button
							...
							style={styles.button}
						>
							{`Zgłoszenie ID: ${zgloszenie.id}, Ulica: ${
								zgloszenie.ulica
							}, Data: ${new Date(
