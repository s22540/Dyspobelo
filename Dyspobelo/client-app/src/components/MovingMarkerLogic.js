...
const MovingMarkerLogic = ({ marker }) => {
    ...
    useEffect(() => {
        ...
        initializeRoute(marker.position, end, markerRef.current, routingControlRef, map);
...
const getRandomCoordinates = (center, range = 500) => {
    ...
    const lngOffset = (Math.random() - 0.5) * range / (40075000 * Math.cos(center[0] * Math.PI / 180) / 360);
...
const initializeRoute = (start, end, marker, routingControlRef, map) => {
    ...
    routingControlRef.current = L.Routing.control({
        ...
        router: L.Routing.mapbox('pk.eyJ1IjoiczIyNTQwIiwiYSI6ImNsdXpwZnVvczB6dzYyam1vMzQ4NnpqZ2sifQ.FUOF_cGUdMx6zorMIiYaQg'),
